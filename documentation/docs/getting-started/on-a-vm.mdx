...
| `TLS_CA_FILE` | no (byoc) | CA that signed `TLS_CERT_FILE`, if not already publicly trusted. Passed to the Workflow Plane so `podman build`/`podman push` can verify `registry.<DOMAIN_BASE>`'s certificate |
...
If the certificate is signed by a CA that isn't already publicly trusted (an internal or corporate CA), also set `TLS_CA_FILE` to that CA's certificate. The installer passes it to the Workflow Plane's build/publish steps so `podman build`/`podman push` can verify the registry's certificate; without it, agent builds fail with `tls: failed to verify certificate: x509: certificate signed by unknown authority` when pulling or pushing images against `registry.<DOMAIN_BASE>`.

```sh
TLS_CA_FILE=/opt/amp/certs/ca.crt   # only if TLS_CERT_FILE isn't signed by a public CA
```
...
| `TLS_CA_FILE` | no (byoc) | CA that signed `TLS_CERT_FILE`, if not already publicly trusted (typical for an internal CA). Passed to the Workflow Plane so `podman build`/`podman push` can verify `registry.<DOMAIN_BASE>`'s certificate. Not needed for `selfsigned` (wired in automatically) or `letsencrypt-dns` (publicly-trusted) |
| `EXTERNAL_GATEWAYS` | no | `true` (default) exposes the `cp` endpoint for external data-plane gateways |
| `HOST_CONSOLE`, `HOST_API`, `HOST_THUNDER`, `HOST_OBSERVER`, `HOST_GATEWAY`, `HOST_CP` | no | Override an individual service hostname (default `<svc>.<DOMAIN_BASE>`) |
| `AGENTS_BASE` | no | Base for deployed-agent hostnames (default `agents.<DOMAIN_BASE>`) |
...
After install, import `/opt/amp/certs/ca.crt` into your clients' trust stores (via MDM/GPO) so browsers trust the console and API without warnings. The installer also passes this CA to the Workflow Plane automatically, so `podman build`/`podman push` can verify `registry.<DOMAIN_BASE>`'s certificate during agent builds — no manual step needed.
...
Since this mode is typically used with an internal CA (not publicly trusted), also set `TLS_CA_FILE` to that CA's certificate so the Workflow Plane's build/publish steps can verify `registry.<DOMAIN_BASE>`'s certificate. Without it, agent builds fail with `tls: failed to verify certificate: x509: certificate signed by unknown authority` when pulling or pushing images against the registry.

```sh
TLS_CA_FILE=/opt/amp/certs/ca.crt   # your internal CA's certificate
```
...
- **Agent builds fail with `x509: certificate signed by unknown authority` pulling/pushing against `registry.<DOMAIN_BASE>`.** The build pipeline (Podman, in the Workflow Plane) doesn't trust the CA that signed the registry's certificate. `selfsigned` mode wires this CA in automatically; in `byoc` mode set `TLS_CA_FILE` to the CA that signed `TLS_CERT_FILE` (see [public](#adv-public-byoc) or [private](#adv-private-byoc)) and re-run the installer so the change reaches the already-installed `amp-platform-resources` release. `letsencrypt`/`letsencrypt-dns` need no action (publicly-trusted CA).
